...
# Note: this version has been partially adapted to work with TrackMate version 7
# which changes the spot analyzer API. Not all features are working yet.
...
def calc_custom_intensity(model, settings, trackmate, feature_int,
							feature_radius, radius, imp, channel=1):
	...
	for spot in all_spots.iterator(False):
		if channel == 1:
			...
			mean = spot.getFeature('MEAN_INTENSITY_CH2')
...
def local_bkgnd_correction_2channel_2radii (model, settings, trackmate, imp, r0, r1):
	...
	calc_custom_intensity(model, settings, trackmate, 'MEAN_INT_R0_C1', 'RADIUS_R0', r0, imps[0], channel=1)
	calc_custom_intensity(model, settings, trackmate, 'MEAN_INT_R1_C1', 'RADIUS_R1', r1, imps[0], channel=1)
	calc_custom_intensity(model, settings, trackmate, 'MEAN_INT_R0_C2', 'RADIUS_R0', r0, imps[1], channel=2)
	calc_custom_intensity(model, settings, trackmate, 'MEAN_INT_R1_C2', 'RADIUS_R1', r1, imps[1], channel=2)
...
def global_bkgnd_correction_2channel (model, settings, trackmate, imp, r0):
	...
	calc_spot_intensity_global_bkgnd (model, settings, trackmate, imps[0],
		'INT_GLOBAL_C1', custom_radius=r0, channel=1)
	calc_spot_intensity_global_bkgnd (model, settings, trackmate, imps[1],
		'INT_GLOBAL_C2', custom_radius=r0, channel=2)
...
def run_trackmate(	img_path,
	...
	# NEED TO FIGURE OUT HOW TO SET CHANNELS FOR THE NEW MULTI-CHANNEL SPOT ANALYZER
	"""
	...
	# Currently not working due to the TrackMate v7 update
	"""
	if bkgnd_corr_settings:
		...
		do_local_bkgnd_corr=False,
		do_global_bkgnd_corr=False,
